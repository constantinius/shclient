...
from .processapi import get_processapi
from .catalogapi import get_catalogapi
...
class ConfigAPIBase(ApiBase):
    ...
    def get_processapi(self, dataset_name=None):
        if dataset_name is not None:
            dataset = self.get_dataset(dataset_name)
            return get_processapi(
                dataset.get('api_endpoint'),
                self.client_id,
                self.client_secret
            )
        else:
            return get_processapi(None, self.client_id, self.client_secret)

    def get_catalog_client(self, dataset_name):
        dataset = self.get_dataset(dataset_name)
        return get_catalogapi(
            dataset.get('api_endpoint'),
            self.client_id,
            self.client_secret
        )
...
class ConfigAPIDefaultLayers(ConfigAPIBase):
    ...
    def __init__(self, client_id, client_secret, api_url=DEFAULT_API_URL,
        ...
        layers_path = layers_path or join(dirname(__file__), 'configs/layers.json')
        ...
        dataproducts_path = dataproducts_path or join(dirname(__file__), 'configs/dataproducts.json')
        if urlparse(layers_path).scheme in ('http', 'https'):
