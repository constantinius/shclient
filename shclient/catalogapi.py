...
def get_catalogapi(api_url, client_id, client_secret, oauth2_url=DEFAULT_OAUTH2_URL):
    api_url = api_url or DEFAULT_API_URL
    if api_url not in CATALOG_CLIENTS:
        CATALOG_CLIENTS[api_url] = CatalogAPI(client_id, client_secret, api_url, oauth2_url)
...
class CatalogAPI(ApiBase):
    ...
    def search(self, collection, bbox_or_geom, time, next_key=None, limit=1000, fields=None, distinct=None, filters=None):
        request_body = {
            ...
            'datetime': '/'.join(iso8601_format(t) for t in time),
            'limit': limit,
            'next': next_key,
        }
